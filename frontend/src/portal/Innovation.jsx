...
import { today, uid, dateLabel } from './data';
import { Badge, Field, Heading, Modal, Panel, Stats, Table, TextArea, formValues } from './ui';
...
export default function Innovation({ section = 'inicio' }) {
  ...
  const sourceNote = <p className="helper">Catálogo tomado de la presentación proporcionada. La convocatoria y los permisos institucionales se validarán antes de abrir registros reales.</p>;
  function register(e) {
    ...
    update('innovation', { ...values, nombre: values.nombre.trim(), id: uid(), user: userId, modulo: catalog.title, categoria: category, etapa: 'Local', estatus: 'Borrador', fecha: today() }); setCategory(null); setError(''); notify('Propuesta guardada en la demostración. Puedes verla en Registros.');
  }
  return <><Heading eyebrow="INNOVATECNM / INNOVACIÓN Y EMPRENDIMIENTO" title={section === 'inicio' ? 'InnovaTecNM' : section === 'registros' ? 'Registro y seguimiento' : catalog.title} description={catalog?.description || 'Cumbre Nacional de Desarrollo Tecnológico, Emprendimiento e Innovación'} action={section !== 'registros' && <Link className="button secondary" to={`${base}/registros${query}`}>Ver registros</Link>} />
    ...
    {detail && <Modal title={detail.nombre} onClose={() => setDetail(null)}><dl className="event-meta"><div><dt>Equipo</dt><dd>{detail.equipo}</dd></div><div><dt>Líder</dt><dd>{detail.lider}</dd></div><div><dt>Asesor</dt><dd>{detail.asesor}</dd></div><div><dt>Registro</dt><dd>{dateLabel(detail.fecha)}</dd></div><div><dt>Categoría</dt><dd>{detail.categoria}</dd></div></dl><p>{detail.descripcion}</p><Badge>{detail.estatus}</Badge>{role === 'admin' && <form className="form-stack" onSubmit={e => { e.preventDefault(); update('innovation', { ...detail, ...formValues(e) }); setDetail(null); notify('Seguimiento actualizado en la demostración.'); }}><Field label="Etapa" name="etapa" defaultValue={detail.etapa}>{['Local', 'Regional', 'Nacional'].map(t => <option key={t}>{t}</option>)}</Field><Field label="Estatus" name="estatus" defaultValue={detail.estatus}>{['Borrador', 'En revisión', 'Aprobada', 'Rechazada'].map(t => <option key={t}>{t}</option>)}</Field><button className="button">Guardar seguimiento</button></form>}</Modal>}
  </>;
}
