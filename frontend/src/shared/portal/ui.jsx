...
export function Badge({ children }) {
  const positive = ['Activo', 'Completado', 'Pagado', 'Confirmada', 'Aprobada', 'Aprobado', 'Disponible'].includes(children);
  const negative = ['Rechazado', 'Rechazada', 'Cancelada', 'Cupo lleno', 'Correcciones solicitadas'].includes(children);
...
export function Modal({ title, children, onClose }) {
  ...
  return <dialog ref={ref} className="modal" aria-labelledby={id} onCancel={onClose}><div className="modal-heading"><h2 id={id}>{title}</h2><button className="icon-button" onClick={onClose} aria-label="Cerrar ventana">×</button></div>{portal?.error && <p className="error" role="alert">{portal.error}</p>}{children}</dialog>;
}
